...
//mark
extern int webfailsafe_upgrade_user_notify;
extern int webfailsafe_upgrade_finished;
extern int webfailsafe_upgrade_downloading;
...
void httpd_appcall(void){
	...
	switch(uip_conn->lport){
		...
		case HTONS(80):
			...
			// new connection
			/*mark add check stat value before reset state*/
			if(uip_connected()){
				httpd_state_reset();
				return;
			}
			...
			if(uip_newdata() && hs->state == STATE_NONE){
				...
				if(hs->state == STATE_FILE_REQUEST){
					...
					printf("Request for: ");
					printf("%s\n", &uip_appdata[4]);
					//PEGA BEGIN : booking finish.html status
					if (strcmp(&uip_appdata[4],"/finish.html") == 0){
					  printf("Finished\n");
					  webfailsafe_upgrade_user_notify = 1;
					}
					//PEGA END<<
					// request for /
					...
					if(start){
					  //PEGA BEGIN : clean udate flag when new uploading file request coming
					  webfailsafe_upgrade_finished = 0;
					  webfailsafe_upgrade_user_notify = 0;
					  //PEGA END<<
					...
					if(!webfailsafe_data_pointer){
						...
						//PEGA<<BEGIN set downloading to identify the download status
						webfailsafe_upgrade_downloading = 1;
						//PEGA>>END
					}
...
int checkimage(void)
{
  ...
  int             rcode = 0;
  int             verify;
  ulong           addr = WEBFAILSAFE_UPLOAD_RAM_ADDRESS;
  return 0;
#if 0

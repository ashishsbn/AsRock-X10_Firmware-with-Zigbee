	...
	if(upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT){
		...
		sprintf(buf,
			"erase 0x%lX +0x%lX; cp.b 0x%lX 0x%lX 0x%lX",
				...
				size);
#else
		        //<<PEGA BEGIN : add fw upgrade command
		                sprintf(buf,
                        "imgaddr=0x%1X && setenv filesize %1X && pchk $imgaddr && source $imgaddr:script",
					WEBFAILSAFE_UPLOAD_RAM_ADDRESS,size);
			//PEGA END>>
#endif

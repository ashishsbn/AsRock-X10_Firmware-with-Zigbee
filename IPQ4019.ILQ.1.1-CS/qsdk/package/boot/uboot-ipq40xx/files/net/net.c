...
#endif
#include "tftp.h"
//<<PEGA BEGIN : include httpd and uip header files
#include "httpd.h"
#include "../include/httpd_def.h"
#include "../httpd/uipopt.h"
#include "../httpd/uip.h"
#include "../httpd/uip_arp.h"
#include <asm/arch-qcom-common/gpio.h>
#ifdef CONFIG_CMD_I2C_LED_MCU
...
//<<PEGA BEGIN : process http packet
#if defined(CONFIG_CMD_HTTPD)
...
//<<PEGA BEGIN : move httpd functions in here
#if defined(CONFIG_CMD_HTTPD)
...
int NetLoopHttpd(void){
	...
	unsigned long led_delay_count = 0;
	unsigned long led_on_blue = 1;
	unsigned long led_on_red = 1;
	unsigned long reset_count_down = 300000;
	unsigned long user_noresponse_count_down = 0;
	int iLoop = 0;
	NetRestarted = 0;
	NetDevExists = 0;
	NetTryCount = 1;
...
restart:
	...
	bi_ip_addr = getenv_IPaddr("failsafeip");
	NetOurIP                = getenv_IPaddr("ipaddr");
	NetOurGatewayIP		= getenv_IPaddr("gatewayip");
	NetOurSubnetMask	= getenv_IPaddr("netmask");
	NetOurVLAN		= getenv_VLAN("vlan");
	...
	// start server...
	printf("HTTP server is starting at IP: %ld.%ld.%ld.%ld\n", (bi_ip_addr & 0xff000000) >> 24, (bi_ip_addr & 0x00ff0000) >> 16, (bi_ip_addr & 0x0000ff00) >> 8,(bi_ip_addr & 0x000000ff));


	HttpdStart();

	// set local host ip address
	ip[1] = ((bi_ip_addr & 0xFFFF0000) >> 16);
	ip[0] = (bi_ip_addr & 0x0000FFFF);

	uip_sethostaddr(ip);

	// set network mask (255.255.255.0 -> local network)
	ip[0] = ((0xFFFFFF00 & 0xFFFF0000) >> 16);
	ip[1] = (0xFFFF00FF & 0x0000FFFF);
	...
	for(;;){
		...
		//PEGA <<BEGIN
		if (webfailsafe_upgrade_downloading == 1){
		  led_ctl(0x0,0xFF,0x0);
		  webfailsafe_upgrade_downloading = 2;
		  printf("turn red led\n");
		}
		if (webfailsafe_upgrade_finished == 0 && webfailsafe_upgrade_downloading == 0){
		  led_delay_count++;
		  if (led_delay_count < 105000){
		    //PEGA<< tunr off led
		    /*
		    ...
		    */
		    if (led_on_blue == 1){
		      led_ctl(0x0,0x0,0xFF);
		      led_on_blue = 0;
		    }
		  }else if (led_delay_count >= 105000 && led_delay_count < 210000){
		    /*
		    ...
		    */
		    if (led_on_red == 1 ){
		      led_ctl(0x00,0xFF,0x00);
		      led_on_red = 0;
		    }
		    //PEGA>>END
		  }else{
		    led_delay_count = 0;
		    led_on_blue = led_on_red = 1;
		  }
		}
		//PEGA BEGIN<< tunr off led
		if (webfailsafe_upgrade_finished == 1 && webfailsafe_upgrade_user_notify == 1){
		  if (reset_count_down <= 0){
		    ...
		    printf("upgrade finished rebooting......\n");
		  }else{
		    reset_count_down--;
		  }
		}else{
		  if (webfailsafe_upgrade_finished == 1){
		    user_noresponse_count_down++;
		    if (user_noresponse_count_down >= 9000000)
		      do_reset(NULL, 0, 0, NULL);
		  }else{
		    user_noresponse_count_down = 0;
		  }
		  reset_count_down=300000;
		}
		//PEGA>>END
		...
		if(do_http_upgrade(NetBootFileXferSize, webfailsafe_upgrade_type) >= 0){
			...
			webfailsafe_upgrade_type = WEBFAILSAFE_UPGRADE_TYPE_FIRMWARE;
			NetBootFileXferSize = 0;
			webfailsafe_upgrade_finished = 1;
			continue;
		}else{
		  //PEGA<< TBD

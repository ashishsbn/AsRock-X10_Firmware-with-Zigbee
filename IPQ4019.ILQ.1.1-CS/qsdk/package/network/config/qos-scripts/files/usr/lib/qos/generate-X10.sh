...
#	local prio="$4"
	...
	config_get type "$section" TYPE
	if [ "$type" = "classify" ]; then
		for option in $options; do
			config_get value "$section" "$option"
			
			if [ "$option" = "dsthost" ]; then
				append "$var" "$prefix" "$N"
				append "$var" "prio 20 u32 match ip dst $value/32"
				append "$var" "$suffix"
			fi
		done
...
add_tc_rules() {
	...
	for rule in $rules; do
		...
		parse_matching_tc_rule tcrule "$rule" "$options" "$prefix" "flowid 1:${target}0"
		append "$var" "$tcrule" "$N"
	done
}
...
get_cg(){
	...
	enum_classes "$cg"
	unset tcIprules
	add_tc_rules tcIprules "$ctrules" "tc filter add dev $device protocol ip parent 1:"
}

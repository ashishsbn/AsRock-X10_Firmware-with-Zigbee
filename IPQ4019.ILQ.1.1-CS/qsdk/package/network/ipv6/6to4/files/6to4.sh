...
proto_6to4_setup() {
	...
	# sync to IPv6 LAN
	uci_set pref-wan global ip6prefix "$prefix6::"
	uci_commit pref-wan

	# add for ipv4 relay server setting
	local ipv4_relay ipv4_relay_auto
	ipv4_relay_auto=$(uci_get pref-wan 6to4 ipv4_relay_auto)
	if [ "$ipv4_relay_auto" == "1" ]; then
		ipv4_relay="192.88.99.1"
	else
		ipv4_relay=$(uci_get pref-wan 6to4 ipv4_relay)
	fi

	if [ "$sourcerouting" != "0" ]; then
		proto_add_ipv6_route "::" 0 "::$ipv4_relay" "" "" "::/128"
		proto_add_ipv6_route "::" 0 "::$ipv4_relay" "" "" "$local6/16"
		proto_add_ipv6_route "::" 0 "::$ipv4_relay" "" "" "$prefix6::/48"
	else
		proto_add_ipv6_route "::" 0 "::$ipv4_relay"
	fi
